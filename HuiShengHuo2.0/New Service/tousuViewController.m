...
@interface tousuViewController ()<UITableViewDelegate,UITableViewDataSource,UITextViewDelegate>
{
    NSArray *dataarr;
...
- (void)createUI
{
    ...
    [but setTitle:@"提交投诉" forState:UIControlStateNormal];
    [but setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    but.titleLabel.font = Font(15);
    [but addTarget:self action:@selector(report) forControlEvents:UIControlEventTouchUpInside];
    [_View addSubview:but];
}
- (void)report
{
    NSString *report_other = TextView.text;
    long i = _tmpBtn.tag;
    
    if (_tmpBtn == nil&&[TextView.text isEqualToString:@""]) {
        [MBProgressHUD showToastToView:self.view withText:@"投诉原因不能为空"];
    }else{
        //初始化进度框，置于当前的View当中
        ...
        [hud showAnimated:YES whileExecutingBlock:^{
            ...
            [manager GET:strurl parameters:dict progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                ...
                WBLog(@"failure--%@",error);
                [MBProgressHUD showToastToView:self.view withText:@"投诉失败"];
            }];
        }// 在HUD被隐藏后的回调
          completionBlock:^{
              //操作执行完后取消对话框
...
-(void)keyWillAppear:(NSNotification *)notification
{
    ...
    CGFloat offset = (_TableView.frame.size.height - kbHeight)-(TextView.frame.origin.y+TextView.frame.size.height+100);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row==0) {
        ...
        imageview.image = [UIImage imageNamed:@"举报商家"];
