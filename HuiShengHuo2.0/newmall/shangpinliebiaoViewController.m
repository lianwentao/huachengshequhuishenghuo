...
@interface shangpinliebiaoViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    NSArray *biaoqianarr;
    UIButton *_tmpBtn;
    NSString *liebiaoid;
...
-(void)postliebiao
{
    ...
    //2.封装参数
    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    NSDictionary *dict = @{@"c_id":[user objectForKey:@"community_id"],@"id":liebiaoid};
    //3.发送GET请求
    /*
     第一个参数:请求路径(NSString)+ 不需要加参数
     第二个参数:发送给服务器的参数数据
...
- (void)postup
{
    ...
    //2.封装参数
    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    NSDictionary *dict = @{@"c_id":[user objectForKey:@"community_id"],@"id":liebiaoid,@"p":[NSString stringWithFormat:@"%d",_pagenum]};
    //3.发送GET请求
    /*
     第一个参数:请求路径(NSString)+ 不需要加参数
     第二个参数:发送给服务器的参数数据
...
- (void)createUI
{
    _TabelView = [[UITableView alloc] initWithFrame:CGRectMake(0, RECTSTATUS.size.height+44+50, Main_width, Main_Height-(RECTSTATUS.size.height+44+50))];
