...
- (void)initUI{
    ...
    self.footerView = [[UIView alloc] initWithFrame:CGRectMake(segmentFrameX, RECTSTATUS.size.height+44+segmentFrameH, segmentFrameW, self.view.bounds.size.height-segmentFrameY-segmentFrameH)];
    [self.view addSubview:self.footerView];
    
    CGRect segmentFrame = CGRectMake(segmentFrameX, RECTSTATUS.size.height+44, segmentFrameW, segmentFrameH);
...
- (void)topBarWithBarView:(CQTopBarView *)segment indexPath:(NSIndexPath *)indexPath{
    ...
    cell.backgroundColor = self.segmentbackColor == nil?[UIColor whiteColor]:self.segmentbackColor;
     [cell.titleImage setImage:[UIImage imageNamed:self.segmentImage==nil?@"ic_arrow_down_grey":self.segmentImage] forState:UIControlStateNormal];
    [self setViewAnimaWithHeight:0];
}
