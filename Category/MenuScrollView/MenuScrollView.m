...
-(void)addViewsWithPageNum:(NSInteger)number withCol:(NSInteger )maxCol withMaxRow:(NSInteger )maxRow{
    ...
    CGFloat btnH =  (self.frame.size.height - 80)/maxRow;
    ...
    for (int i = 0; i<indexCount; i++) {
        ...
        CGFloat btnY =  row * (btnH+15);
        UIView * view = [[UIView alloc] initWithFrame:CGRectMake(btnX, btnY+15, btnW, btnH)];
...
- (UIPageControl *)pageControl{
    if (!_pageControl) {
        ...
        _pageControl.pageIndicatorTintColor = BackColor;
        _pageControl.currentPageIndicatorTintColor = QIColor;
